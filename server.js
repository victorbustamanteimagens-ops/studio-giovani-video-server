...
// Corta cada item {path, inicio, fim}, normaliza pra 1080x1920/30fps/yuv420p
...
async function trimAndConcat(items, jobId) {
  ...
  var temps = trimmedPaths.concat([concatListPath]);
  ...
  var concatArgs = ['-y', '-f', 'concat', '-safe', '0', '-i', concatListPath, '-c', 'copy', '-movflags', '+faststart', outputPath];
...
function buildEditPrompt(clips, targetSeconds) {
  ...
  return [
    ...
    '',
    '1. ASSISTA TUDO ANTES DE DECIDIR. Identifique o ambiente de cada clipe, o movimento de câmera (push-in, pull-out, pan pra esquerda/direita, tilt, travelling lateral, parado) e dê uma nota visual de 0 a 10 (luz, nitidez, estabilidade, quão vendável é o ambiente).',
    '',
    ...
    '',
    'Regras dos números: "inicio" e "fim" em segundos com uma casa decimal, dentro da duração real do clipe (0 ≤ inicio < fim ≤ duração). "ordem" é a posição no vídeo final: 0 = hook, depois 1, 2, 3… consecutivos, sem repetir; clipes com "incluir": false recebem "ordem": -1.',
    '',
...
// dentro da duração real, hook curto, ordem sem buraco, mínimo de clipes.
function normalizePlan(parsed, clips) {
  var n = clips.length;
  ...
  var minIncl = Math.min(n, Math.max(2, Math.ceil(n * 0.7)));
  if (included.length < minIncl) {
    // a IA cortou demais — devolve os melhores excluídos até bater o mínimo
  ...
  if (included.length && included[0].fim - included[0].inicio > 2.5) {
    included[0].fim = Math.round((included[0].inicio + 2.2) * 100) / 100;
  }
...
app.post('/autoedit/smart', requireSiteOrigin, function (req, res, next) {
  ...
  next();
}, smartEditLimiter, uploadClips.array('clips', 10), async function (req, res) {
  var clipFiles = req.files || [];
  ...
  function cleanupInputs() {
    return cleanupFiles(clipFiles).then(function () { return cleanupPaths(proxyPaths); });
  }
  ...
  try {
    ...
    var targetSeconds = Math.round(Math.min(35, Math.max(10, Math.min(totalRaw * 0.6, clips.length * 2.8))));

    // 2) IA monta a edição
    var parts = [{ text: buildEditPrompt(clips, targetSeconds) }];
    ...
    var result = normalizePlan(parsed, clips);
    ...
    // 3) corta os ORIGINAIS na ordem da montagem e junta
    var items = result.plano.map(function (e) {
      return { path: clipFiles[e.indice].path, inicio: e.inicio, fim: e.fim };
    });
    var out = await trimAndConcat(items, jobId);
    if (!out.ok) {
      await cleanupInputs();
    ...
    var planPayload = {
      ...
      duracaoFinal: Math.round(result.plano.reduce(function (s, e) { return s + (e.fim - e.inicio); }, 0) * 10) / 10,
      plano: result.plano.map(function (e) {
        return { indice: e.indice, nome: clips[e.indice].nome, ambiente: e.ambiente, movimento: e.movimento, nota: e.nota, inicio: e.inicio, fim: e.fim, motivo: e.motivo };
      }),
